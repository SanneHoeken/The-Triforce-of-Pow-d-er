...
class CharlotteProteinFolder():
    ...
    def fold(self, fold_position = 0):
        ...
        for node in non_visited_nodes:
            ...
            if node.depth >= len(self.source_protein.get_aminos()):
                break
            ...
            if current_amino is not None:
                ...
                folds = self.get_possible_folds(protein, x, y)
                
                # Goes through all possible folds from current protein
                for fold in folds:
                    new_amino = copy.deepcopy(current_amino)
                    new_amino.type = self.source_protein.get_aminos()[node.depth].type
